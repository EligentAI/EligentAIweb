      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        <div className="py-14 grid grid-cols-1 md:grid-cols-3 gap-10">
          ...
          <div>
            ...
            <p className="text-[13px] leading-relaxed max-w-[240px]" style={{ color: "var(--color-text-secondary)" }}>{t.footer.tagline}</p>
          </div>

          {/* Navigation */}
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-widest mb-4" style={{ color: "var(--color-text-muted)" }}>{t.footer.navTitle}</p>
            <div className="grid grid-cols-2 gap-x-6 gap-y-2">
              {navLinks.map((link) => (
                <button key={link.href} onClick={() => scrollTo(link.href)}
                  className="text-left text-[13px] py-0.5 hover:text-mint transition-colors" style={{ color: "var(--color-text-secondary)" }}>
                  {link.label}
                </button>
              ))}
            </div>
          ...
          {/* Connect */}
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-widest mb-4" style={{ color: "var(--color-text-muted)" }}>{t.footer.connectTitle}</p>
        ...
        <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-4" style={{ borderTop: "1px solid var(--color-border)" }}>
          ...
          <button onClick={() => scrollTo("#home")}
            className="flex items-center gap-2 text-[12px] hover:text-mint transition-colors group" style={{ color: "var(--color-text-muted)" }}>
            {t.footer.backTop}
            <div className="w-6 h-6 rounded-md flex items-center justify-center transition-all group-hover:border-mint/25 group-hover:bg-mint/05"
